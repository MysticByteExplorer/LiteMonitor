...
namespace LiteMonitor
{
    public class MainForm : Form
    {
        ...
        // ==== FPS屏幕显示 ====
        private FpsScreenForm? _fpsScreenForm;
        ...
        // 控制FPS屏幕显示
        public void ToggleFpsScreen(bool show)
        {
            if (show)
            {
                if (_fpsScreenForm == null || _fpsScreenForm.IsDisposed)
                {
                    // 创建FPS屏幕显示窗体
                    var hardwareMonitor = new HardwareMonitor(_cfg);
                    _fpsScreenForm = new FpsScreenForm(_cfg, hardwareMonitor);
                    _fpsScreenForm.Show();
                }
                else
                {
                    if (!_fpsScreenForm.Visible)
                        _fpsScreenForm.Show();
                }
            }
            else
            {
                if (_fpsScreenForm != null)
                {
                    _fpsScreenForm.Close();
                    _fpsScreenForm.Dispose();
                    _fpsScreenForm = null;
                }
            }
        }
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            // === 根据配置启动FPS屏幕显示 ===
            if (_cfg.ShowFpsOnScreen)
            {
                ToggleFpsScreen(true);
            }
