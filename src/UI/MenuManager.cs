...
namespace LiteMonitor
{
    public static class MenuManager
    {
        ...
        public static ContextMenuStrip Build(MainForm form, Settings cfg, UIController? ui)
        {
            ...
            // === FPS屏幕显示 ===
            var fpsScreenRoot = new ToolStripMenuItem(LanguageManager.T("Menu.FpsScreen"));
            
            // 开关选项
            var fpsScreenToggle = new ToolStripMenuItem(LanguageManager.T("Menu.FpsScreenToggle"))
            {
                Checked = cfg.ShowFpsOnScreen,
                CheckOnClick = true
            };
            fpsScreenToggle.CheckedChanged += (_, __) =>
            {
                cfg.ShowFpsOnScreen = fpsScreenToggle.Checked;
                cfg.Save();
                form.ToggleFpsScreen(cfg.ShowFpsOnScreen);
            };
            fpsScreenRoot.DropDownItems.Add(fpsScreenToggle);
            fpsScreenRoot.DropDownItems.Add(new ToolStripSeparator());
            
            // 位置选择
            var fpsPositionRoot = new ToolStripMenuItem(LanguageManager.T("Menu.FpsPosition"));
            
            // 左上
            var posTopLeft = new ToolStripMenuItem(LanguageManager.T("Menu.FpsTopLeft"))
            {
                Checked = cfg.FpsScreenPosition == (int)FpsScreenPosition.TopLeft
            };
            posTopLeft.Click += (_, __) => SetFpsPosition((int)FpsScreenPosition.TopLeft);
            
            // 中上
            var posTopCenter = new ToolStripMenuItem(LanguageManager.T("Menu.FpsTopCenter"))
            {
                Checked = cfg.FpsScreenPosition == (int)FpsScreenPosition.TopCenter
            };
            posTopCenter.Click += (_, __) => SetFpsPosition((int)FpsScreenPosition.TopCenter);
            
            // 右上
            var posTopRight = new ToolStripMenuItem(LanguageManager.T("Menu.FpsTopRight"))
            {
                Checked = cfg.FpsScreenPosition == (int)FpsScreenPosition.TopRight
            };
            posTopRight.Click += (_, __) => SetFpsPosition((int)FpsScreenPosition.TopRight);
            
            fpsPositionRoot.DropDownItems.AddRange(new[] { posTopLeft, posTopCenter, posTopRight });
            fpsScreenRoot.DropDownItems.Add(fpsPositionRoot);
            modeRoot.DropDownItems.Add(fpsScreenRoot);
            
            // FPS位置设置辅助函数
            void SetFpsPosition(int position)
            {
                cfg.FpsScreenPosition = position;
                cfg.Save();
                // 如果FPS屏幕显示已开启，需要更新位置
                if (cfg.ShowFpsOnScreen)
                {
                    // 重新创建窗体以应用新位置
                    form.ToggleFpsScreen(false);
                    form.ToggleFpsScreen(true);
                }
            }
