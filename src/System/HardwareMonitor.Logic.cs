    ...
    public sealed partial class HardwareMonitor
    {
        ...
        public float? Get(string key)
        {
            ...
            if (key == "FPS.Current")
            {
                ...
                // 如果游戏帧率无效（比如没有游戏运行），则使用系统刷新率
                if (gameFps < 1 || float.IsNaN(gameFps) || float.IsInfinity(gameFps))
                {
                    gameFps = GetSystemRefreshRate();
                }
        ...
        // 性能计数器辅助类
        private static class FpsPerformanceCounter
        {
            private static long frequency;
            private static long startTime;

            static FpsPerformanceCounter()
            {
                // 初始化高性能计时器
            ...
            public static long FrameCount
            {
                get
                {
                    ...
                    return currentTime / (frequency / 60); // 简单估算，每60分之一秒算一帧
                }
            }
            ...
            public static double ElapsedSeconds
            {
                get
                {
                    long currentTime;
                    QueryPerformanceCounter(out currentTime);
                    return (currentTime - startTime) / (double)frequency;
                }
            }
        ...
        private const int VREFRESH = 116; // 垂直刷新率

        // 用于跟踪实际帧率的变量
        private long lastFrameCount = 0;
        private double lastTime = 0;

        // 获取游戏实际帧率的方法（基于Performance Counter）
        private float GetGameFps()
        {
            try
            {
                // 使用Performance Counter获取高精度时间
                long currentFrameCount = FpsPerformanceCounter.FrameCount;
                double currentTime = FpsPerformanceCounter.ElapsedSeconds;
                ...
                if (timeDelta > 0)
                {
                    fps = (float)(frameDelta / timeDelta);
                }
