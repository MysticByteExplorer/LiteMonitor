...
namespace LiteMonitor.src.UI.SettingsPage
{
    public class MainPanelPage : SettingsPageBase
    {
        ...
        private void CreateFpsScreenCard()
        {
            var group = new LiteSettingsGroup("FPS屏幕显示设置");
            
            // 1. FPS屏幕显示开关
            AddBool(group, "Menu.FpsScreenToggle", 
                () => Config.ShowFpsOnScreen, 
                v => Config.ShowFpsOnScreen = v);
            
            // 2. FPS显示位置
            AddComboIndex(group, "Menu.FpsPosition", 
                new[] { "左上角", "顶部中央", "右上角" },
                () => Config.FpsScreenPosition, 
                idx => Config.FpsScreenPosition = idx);
            
            // 3. FPS文字大小
            AddNumberDouble(group, "FPS文字大小", "pt", 
                () => Config.FpsScreenFontSize, 
                v => Config.FpsScreenFontSize = (float)v);
            
            // 4. FPS显示格式
            var formatInput = new LiteUnderlineInput(Config.FpsScreenFormat);
            Bind(formatInput, () => Config.FpsScreenFormat, v => Config.FpsScreenFormat = v);
            group.AddItem(new LiteSettingsItem("显示格式", formatInput));
            group.AddFullItem(new LiteNote("支持 $[fps] 格式，例如：FPS: $[fps]", 0));
            
            // 5. FPS显示颜色
            var colorInput = new LiteColorInput(Config.FpsScreenColor);
            Bind(colorInput, () => Config.FpsScreenColor, v => Config.FpsScreenColor = v);
            group.AddItem(new LiteSettingsItem("显示颜色", colorInput));
            
            AddGroupToPage(group);
        }
        
        // 添加字符串绑定方法
        private void Bind(LiteUnderlineInput input, Func<string> getter, Action<string> setter)
        {
        ...
        // 添加颜色输入绑定方法
        private void Bind(LiteColorInput input, Func<string> getter, Action<string> setter)
        {
