...
namespace LiteMonitor.src.Core
{
    ...
    public static class AppActions
    {
        ...
        public static void ApplyAllSettings(Settings cfg, MainForm mainForm, UIController ui)
        {
            ...
            // 6. FPS屏幕位置更新
            UpdateFpsScreenPosition(cfg, mainForm);
        }
        ...
        // 更新FPS屏幕位置
        private static void UpdateFpsScreenPosition(Settings cfg, MainForm mainForm)
        {
            foreach (Form f in Application.OpenForms)
            {
                if (f is FpsScreenForm fpsForm)
                {
                    // 更新位置
                    fpsForm.SetPosition((FpsScreenPosition)cfg.FpsScreenPosition);
                    // 更新颜色（如果需要）
                    // 这里暂时不处理，因为颜色会在FPS窗体的定时器中自动更新
                }
            }
        }
    }
